...
Partial Class Modules_ucSubBillingPayment
    ...
    Protected Sub AddPolicyNo_Click(sender As Object, e As EventArgs) Handles AddPolicyNo.Click
        ...
        Using dc As New DataClasses_GoodWorldExt()
            ...
            Dim _newData As New tblSubBillingPayment
            With _newData
            ...
            dc.tblSubBillingPayments.InsertOnSubmit(_newData)
            dc.SubmitChanges()
    ...
    Protected Sub PolicyNo_ItemRequestedByValue(source As Object, e As ListEditItemRequestedByValueEventArgs) Handles PolicyNoFilter.ItemRequestedByValue
        ...
        SqlDataSource1.SelectCommand = "select PolicyNo from v_SubBillingDetails where v_SubBillingDetails.PolicyID not in (select PolicyID from tblSubBillingPayment) and PolicyNo <> '' and  PolicyNo like @PolicyNo and isnull(PolicyNo,'') <> ''"
    ...
    Protected Sub PolicyNo_ItemsRequestedByFilterCondition(source As Object, e As ListEditItemsRequestedByFilterConditionEventArgs) Handles PolicyNoFilter.ItemsRequestedByFilterCondition
        ...
        sb.Append(" SELECT st.* ")
        sb.Append(" FROM (")
        sb.Append(" select v_SubBillingDetails.* ")
        sb.Append(" , row_number()over(order by v_SubBillingDetails.PolicyNo) as [rn] ")
        sb.Append(" from v_SubBillingDetails ")
        sb.Append(" where v_SubBillingDetails.PolicyID not in (select PolicyID from tblSubBillingPayment) and PolicyNo <> '' and PolicyNo LIKE @filter and isnull(PolicyNo,'') <> '' ")
        sb.Append(" ) as st ")
