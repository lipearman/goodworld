...
Partial Class Modules_ucBillingPayment
    ...
    Protected Sub AddPolicyNo_Click(sender As Object, e As EventArgs) Handles AddPolicyNo.Click
        ...
        Using dc As New DataClasses_GoodWorldExt()
            ...
            Dim _newData As New tblBillingPayment
            With _newData
            ...
            dc.tblBillingPayments.InsertOnSubmit(_newData)
            dc.SubmitChanges()
    ...
    Protected Sub PolicyNo_ItemRequestedByValue(source As Object, e As ListEditItemRequestedByValueEventArgs) Handles PolicyNoFilter.ItemRequestedByValue
        ...
        SqlDataSource1.SelectCommand = "select PolicyNo from v_Report1 where v_Report1.ID not in (select PolicyID from tblBillingPayment) and PolicyNo <> '' and  PolicyNo like @PolicyNo and isnull(PolicyNo,'') <> ''"
    ...
    Protected Sub PolicyNo_ItemsRequestedByFilterCondition(source As Object, e As ListEditItemsRequestedByFilterConditionEventArgs) Handles PolicyNoFilter.ItemsRequestedByFilterCondition
        ...
        sb.Append(" where v_Report1.ID not in (select PolicyID from tblBillingPayment) and PolicyNo <> '' and PolicyNo LIKE @filter and isnull(PolicyNo,'') <> '' ")
        sb.Append(" ) as st ")
