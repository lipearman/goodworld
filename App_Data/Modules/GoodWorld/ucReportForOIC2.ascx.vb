...
Partial Class Modules_ucReportForOIC2
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        Dim _MonthlyText = MonthlyName.Text
        Dim _MonthlyValue = MonthlyName.Value.ToString().Split("-")
        Dim _MonthlyDate = New Date(_MonthlyValue(0), _MonthlyValue(1), _MonthlyValue(2))
        ...
        sb.Append(" SELECT ROW_NUMBER() OVER(ORDER BY EffectiveDate ) AS RowNo,* ")
        sb.Append(" FROM v_Report1 ")
        sb.AppendFormat(" where Monthly is not null and  convert(varchar,Monthly,112) = '{0}'", MonthlyName.Value.ToString().Replace("-", ""))
        sb.AppendFormat(" order by EffectiveDate")

        Dim ReportFile = "Report4.rdl"
        ...
        ReportViewer1.LocalReport.SetParameters(New ReportParameter("monthlytext", String.Format("ประจำเดือน {0}", MyUtils.GenThaiMonthlyDate(_MonthlyDate, 2))))
